...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const s of ouverts) {
      ...
      /* Liste des inscrits payés. On ne sort QUE le prénom et les jours :
         jamais le nom de famille, la date de naissance ni les coordonnées.
         Les parents ont consenti à l'affichage du prénom lors de la licence. */
      let inscrits = 0, prenoms = [];
      try {
        const payees = await listerPayees(s.id);
        inscrits = payees.length;
        prenoms = payees.slice(0, 60).map((i) => ({
          prenom: String(i.enfant?.prenom || "").trim(),
          jours: i.jours,
        })).filter((x) => x.prenom);
    ...
    // cache très court, et pas de version périmée servie longtemps.
