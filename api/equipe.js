...
function transformer(engagement, poule) {
  ...
  /* Déduplication : une rencontre reportée peut apparaître deux fois.
     On garde la dernière version rencontrée pour un même identifiant,
     et à défaut d'identifiant, une même journée contre un même
     adversaire à la même date. */
  const vues = new Map();
  ...
  const nos = [...vues.values()].sort((a, b) =>
...
export default async function handler(req, res) {
  ...
  // Mode exploration : /api/equipe?id=...&champs=1
  // Récupère TOUS les champs bruts des rencontres de la poule pour
  // repérer ce qui distingue deux calendriers cohabitant dans la
  // même poule (annulation, statut, date de saisie, sous-poule...).
  if (req.query?.champs === "1") {
    try {
      const engagement = await directus(
        `/items/ffbbserver_engagements/${encodeURIComponent(id)}`,
        queryString(["id", "nom", "idPoule.id", "idPoule.nom"])
      );
      const pouleId = engagement?.idPoule?.id;
      const p = new URLSearchParams();
      p.append("fields[]", "id");
      p.append("fields[]", "nom");
      p.append("fields[]", "rencontres.*");
      p.append("deep", JSON.stringify({ rencontres: { _limit: 200, _sort: ["numeroJournee"] } }));

      const poule = await directus(`/items/ffbbserver_poules/${encodeURIComponent(pouleId)}`, p.toString());
      const rs = Array.isArray(poule?.rencontres) ? poule.rencontres : [];

      // On isole les journées qui apparaissent plusieurs fois
      const parJournee = {};
      for (const r of rs) {
        const j = String(r.numeroJournee);
        (parJournee[j] = parJournee[j] || []).push(r);
      }
      const doublons = Object.entries(parJournee)
        .filter(([, v]) => v.length > 2)   // plus de 2 rencontres pour une journée dans une poule de 2 équipes par match
        .slice(0, 2);

      res.setHeader("Cache-Control", "no-store");
      res.status(200).json({
        poule: { id: poule?.id, nom: poule?.nom },
        rencontresTotal: rs.length,
        champsDisponibles: rs.length ? Object.keys(rs[0]).sort() : [],
        exemple: rs[0] || null,
        journeesChargees: Object.keys(parJournee).length,
        exemplesDoublons: doublons.map(([j, v]) => ({
          journee: j,
          nombre: v.length,
          rencontres: v.slice(0, 4),
        })),
      });
    } catch (e) {
