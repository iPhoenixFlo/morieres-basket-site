...
const CHAMPS_POULE = [
  ...
  // Dates de saisie dans FBI : servent à écarter un calendrier annulé
  // qui cohabiterait avec le nouveau dans la même poule.
  "rencontres.creation",
  "rencontres.date_created",
  "rencontres.joue",
...
function transformer(engagement, poule) {
  const brutes = Array.isArray(poule?.rencontres) ? poule.rencontres : [];
  const monEngagement = String(engagement?.id || "");

  /* On filtre sur l'ENGAGEMENT et non sur le code du club : le code
     SUD0084026 est commun à toutes les équipes du club, donc deux
     équipes dans la même poule produiraient des journées en double.
     Le code club ne sert que de secours si l'engagement manque. */
  const concerne = (r) => {
    const e1 = String(r?.idEngagementEquipe1?.id || "");
    const e2 = String(r?.idEngagementEquipe2?.id || "");
    if (e1 || e2) return e1 === monEngagement || e2 === monEngagement;
  ...
  /* Déduplication stricte : une même rencontre renvoyée deux fois. */
  const vues = new Map();
  for (const r of brutes) {
    if (!concerne(r)) continue;
    const cle = r?.id
      ? `id:${r.id}`
      : `j:${r?.numeroJournee}|${r?.nomEquipe1}|${r?.nomEquipe2}|${r?.date_rencontre}`;
    vues.set(cle, r);
  }

  /* Purge des calendriers fantômes.

     Le comité peut publier un calendrier, l'annuler, puis en publier un
     second sans supprimer le premier. Les deux cohabitent alors dans la
     même poule, sous le même engagement : ni le filtre sur l'engagement
     ni la déduplication par identifiant ne les distinguent. On se
     retrouve avec la bonne affiche à la mauvaise date, et une journée
     qui apparaît deux fois.

     Le discriminant est la date de saisie dans FBI. Un calendrier est
     généré d'un bloc, donc toutes ses rencontres partagent la même
     date de création, à la seconde près. On regroupe donc les
     rencontres par affiche (les deux mêmes équipes, dans le même sens,
     donc même receveur) et on ne garde que celles issues de la saisie
     la plus récente. Pas de pari sur laquelle est la bonne : c'est la
     dernière saisie qui fait foi, et la règle tiendra si le comité
     recommence l'an prochain.

     À défaut de date de saisie, on se rabat sur l'identifiant le plus
     élevé, la rencontre la plus récemment créée. */
  const horodatageSaisie = (r) => {
    const v = r?.creation || r?.date_created || null;
    if (!v) return 0;
  ...
  const cleAffiche = (r) => {
    const c1 =
      r?.idEngagementEquipe1?.id ||
      r?.idEngagementEquipe1?.idOrganisme?.code ||
      r?.nomEquipe1 || "?";
    const c2 =
      r?.idEngagementEquipe2?.id ||
      r?.idEngagementEquipe2?.idOrganisme?.code ||
      r?.nomEquipe2 || "?";
    return `${c1}>${c2}`;
  };

  const affiches = new Map();
  for (const r of vues.values()) {
    const k = cleAffiche(r);
    if (!affiches.has(k)) affiches.set(k, []);
    affiches.get(k).push(r);
  }

  const retenues = [];
  const ecartees = [];

  for (const lot of affiches.values()) {
    if (lot.length === 1) {
      retenues.push(lot[0]);
      continue;
    }
    const parRecence = [...lot].sort(
      (a, b) =>
        horodatageSaisie(b) - horodatageSaisie(a) ||
        (Number(b?.id) || 0) - (Number(a?.id) || 0)
    );
    const reference = horodatageSaisie(parRecence[0]);
    // Si la date de saisie est connue, on garde toutes les rencontres
    // du même lot de saisie. Sinon, la plus récemment créée seulement.
    const garde =
      reference > 0
        ? parRecence.filter((r) => horodatageSaisie(r) === reference)
        : [parRecence[0]];

    retenues.push(...garde);
    for (const r of parRecence) if (!garde.includes(r)) ecartees.push(r);
  }

  const nos = retenues.sort((a, b) =>
    String(a?.date_rencontre || "").localeCompare(String(b?.date_rencontre || ""))
  );

  // Trace des rencontres écartées, pour vérifier d'un coup d'oeil que
  // ce sont bien des fantômes et rien d'autre.
  const journalEcartees = ecartees
    .map((r) => ({
      id: r?.id ?? null,
      numero: r?.numero ?? null,
      journee: r?.numeroJournee ?? null,
      date: r?.date_rencontre ?? null,
      affiche: `${r?.nomEquipe1 || "?"} / ${r?.nomEquipe2 || "?"}`,
      saisieLe: r?.creation || r?.date_created || null,
    }))
  ...
  return {
    ...
    resume: {
      ...
      ecartees: journalEcartees.length,
    ...
    calendrier: matchs,
    classement,
    // Vide en temps normal. Non vide = un calendrier annulé traîne
    // encore dans la base du comité.
    ecartees: journalEcartees,
